...
module internal FSharp.Compatibility.OCaml.Printf

//let format_float: string -> float -> string
//  = "caml_format_float"
//let format_int: string -> int -> string
//  = "caml_format_int"
//let format_int32: string -> int32 -> string
//  = "caml_int32_format"
//let format_nativeint: string -> nativeint -> string
//  = "caml_nativeint_format"
//let format_int64: string -> int64 -> string
//  = "caml_int64_format"

//
let format_float (fmt : string) (value : float) : string =
    raise <| System.NotImplementedException "Printf.format_float"

//
let format_int (fmt : string) (value : int) : string =
    raise <| System.NotImplementedException "Printf.format_int"

//
let format_int32 (fmt : string) (value : int32) : string =
    raise <| System.NotImplementedException "Printf.format_int32"

//
let format_nativeint (fmt : string) (value : nativeint) : string =
    raise <| System.NotImplementedException "Printf.format_nativeint"

//
let format_int64 (fmt : string) (value : int64) : string =
    raise <| System.NotImplementedException "Printf.format_int64"
...
module Sformat =
  ...
  let length (format : ('a, 'b, 'c, 'd, 'e, 'f) format6) : int =
    String.length format.RawValue
  
  let get (format : ('a, 'b, 'c, 'd, 'e, 'f) format6) (pos : int) : char =
    format.RawValue.[pos]
  
  let unsafe_get (format : ('a, 'b, 'c, 'd, 'e, 'f) format6) (pos : int) : char =
    format.RawValue.[pos]
  
  let unsafe_to_string (format : ('a, 'b, 'c, 'd, 'e, 'f) format6) : string =
    format.RawValue
...
let parse_string_conversion sfmt =
  let rec parse neg i =
    if i >= (String.length sfmt)
    then (0, neg)
    else
      (match String.get sfmt i with
       | x when x >= '1' && x <= '9' ->
           ((int_of_string
       ...
       | '-' -> parse true (succ i)
       | _ -> parse neg (succ i))
  in try parse false 1 with | Failure _ -> bad_conversion sfmt 0 's'
...
let pad_string pad_char p neg s i len =
  if p = len && i = 0 then s
        ...
        let substr = String.sub s i len
        if neg then
            substr.PadRight (p, pad_char)
        else
            substr.PadLeft (p, pad_char)
  
(* Format a string given a %s format, e.g. %40s or %-20s.
   To do ?: ignore other flags (#, +, etc). *)
let format_string sfmt s =
  let (p, neg) = parse_string_conversion sfmt
  in pad_string ' ' p neg s 0 (String.length s)
    ...
    | x when x >= '0' && x <= '9' ->
        let rec skip_int_literal i =
          (match Sformat.unsafe_get fmt i with
           | x when x >= '0' && x <= '9' -> skip_int_literal (succ i)
           | '$' -> succ i
           | _ -> start)
        in skip_int_literal (succ start)
    | _ -> start in
  let start = skip_positional_spec (succ start) in
  let b = Buffer.create ((stop - start) + 10)
  in
    (Buffer.add_char b '%';
         ...
         (match ((Sformat.unsafe_get fmt i), widths) with
          | ('*', h :: t) ->
              (Buffer.add_string b (string_of_int h);
               let i = skip_positional_spec (succ i) in fill_format i t)
          | ('*', []) -> assert false
          | (* Should not happen since this is ill-typed. *) (c, _) ->
              (Buffer.add_char b c; fill_format (succ i) widths))
       else ()
     in (fill_format start (List.rev widths); Buffer.contents b))
  
let extract_format_int conv fmt start stop widths =
   let sfmt = extract_format fmt start stop widths in
   match conv with
   | 'n' | 'N' ->
     let sfmt = sfmt.Remove (String.length sfmt - 1)
     sfmt + "u"
   | _ -> sfmt
  ...
  let rec sub_fmt c i =
    ...
    and sub_sub j =
      ...
      then incomplete_format fmt
        ...
        (match Sformat.get fmt j with
         | ('(' | '{' as c) -> let j = sub_fmt c (succ j) in sub (succ j)
         | ('}' | ')' as c) ->
  ...
  let rec scan_flags skip i =
    ...
    then incomplete_format fmt
      ...
      (match Sformat.unsafe_get fmt i with
       ...
       | x when x >= '0' && x <= '9' -> scan_flags skip (succ i)
       | '.' -> scan_flags skip (succ i)
       | _ -> scan_conv skip i)
    ...
    then incomplete_format fmt
      ...
      (match Sformat.unsafe_get fmt i with
       ...
       | ('{' as conv) ->
           ...
           let rec loop i =
             if i < (j - 2)
             then loop (add_char i (Sformat.get fmt i))
             else ()
           in
             (loop i;
...
let summarize_format_type fmt =
  ...
  let add_conv skip i c =
    (if skip then Buffer.add_string b "%_" else Buffer.add_char b '%';
     add_char i c)
  in (iter_on_format_args fmt add_conv add_char; Buffer.contents b)
...
let ac_of_format fmt =
  let ac = { ac_rglr = 0; ac_skip = 0; ac_rdrs = 0; }
  let incr_ac skip c =
    let inc = if c = 'a' then 2 else 1
    if c = 'r' then ac.ac_rdrs <- ac.ac_rdrs + 1
    if skip
    then ac.ac_skip <- ac.ac_skip + inc
    else ac.ac_rglr <- ac.ac_rglr + inc
  let rec add_conv skip i c =
    (* Just finishing a meta format: no additional argument to record. *)
    if (c <> ')') && (c <> '}') then incr_ac skip c
    succ i
  and add_char i _ = succ i
  in (iter_on_format_args fmt add_conv add_char; ac)
  
let count_arguments_of_format fmt =
  let ac = ac_of_format fmt
  in
...
let list_iter_i f l =
  let rec loop i =
    ...
    | [] -> ()
    | [ x ] -> f i x
    | (* Tail calling [f] *) x :: xs -> (f i x; loop (succ i) xs)
  in loop 0 l
...
type positional_specification = | Spec_none | Spec_index of Sformat.index
  ...
  | d when d >= '0' && d <= '9' ->
      let rec get_int_literal accu j =
        (match Sformat.unsafe_get fmt j with
         ...
         | '$' ->
             if accu = 0
             then failwith "printf: bad positional specification (0)."
             else
               got_spec (Spec_index (Sformat.index_of_literal_position accu))
                 (succ j)
      ...
      in get_int_literal ((int_of_char d) - 48) (succ i)
...
let next_index spec n =
  match spec with | Spec_none -> Sformat.succ_index n | Spec_index _ -> n
  
(* Get the index of the actual argument to printf, according to its
   optional positional specification. *)
let get_index spec n = match spec with | Spec_none -> n | Spec_index p -> p
  ...
  let make_valid_float_lexeme s =
    ...
    let rec valid_float_loop i =
      ...
      then s ^ "."
        ...
        (match s.[i] with
         | (* Sure, this is already a valid float lexeme. *) '.' | 'e' | 'E'
             -> s
         | _ -> valid_float_loop (i + 1))
    in valid_float_loop 0
  in
    fun sfmt x ->
      let s = format_float sfmt x
      in
...
let scan_format fmt args n pos cont_s cont_a cont_t cont_f cont_m =
  ...
  let rec scan_positional n widths i =
    let got_spec spec i = scan_flags spec n widths i
    in scan_positional_spec fmt got_spec i
  and scan_flags spec n widths i =
    match Sformat.unsafe_get fmt i with
    | '*' ->
        let got_spec wspec i =
          let (width : int) = get_arg wspec n
          in scan_flags spec (next_index wspec n) (width :: widths) i
        in scan_positional_spec fmt got_spec (succ i)
    | x when x >= '0' && x <= '9' ->
  ...
  and scan_conv spec n widths i =
    match Sformat.unsafe_get fmt i with
    | ('%' | '@' as c) -> cont_s n (String.make 1 c) (succ i)
    | '!' -> cont_f n (succ i)
    | ',' -> cont_s n "" (succ i)
    | ('s' | 'S' as conv) ->
        let (x : string) = get_arg spec n in
        let x =
          if conv = 's' then x else "\"" ^ ((String.escaped x) ^ "\"") in
        let s = (* Optimize for common case %s *)
          if i = (succ pos)
          then x
          else format_string (extract_format fmt pos i widths) x
        in cont_s (next_index spec n) s (succ i)
    | ('[' as conv) -> bad_conversion_format fmt i conv
    | ('c' | 'C' as conv) ->
        let (x : char) = get_arg spec n in
        let s =
          if conv = 'c'
          then String.make 1 x
          else "'" ^ ((Char.escaped x) ^ "'")
        in cont_s (next_index spec n) s (succ i)
    | ('d' | 'i' | 'o' | 'u' | 'x' | 'X' | 'N' as conv) ->
        let (x : int) = get_arg spec n in
        let s = format_int (extract_format_int conv fmt pos i widths) x
        in cont_s (next_index spec n) s (succ i)
    | 'f' | 'e' | 'E' | 'g' | 'G' ->
        let (x : float) = get_arg spec n in
        let s = format_float (extract_format fmt pos i widths) x
        in cont_s (next_index spec n) s (succ i)
    | ('F' as conv) ->
        let (x : float) = get_arg spec n in
        let s =
          if widths = []
          then Pervasives.string_of_float x
          else
            format_float_lexeme (extract_format_float conv fmt pos i widths)
              x
        in cont_s (next_index spec n) s (succ i)
    | 'B' | 'b' ->
        let (x : bool) = get_arg spec n
        in cont_s (next_index spec n) (string_of_bool x) (succ i)
    | 'a' ->
        ...
        let arg = get_arg Spec_none n
        in cont_a (next_index spec n) printer arg (succ i)
    | ('r' as conv) -> bad_conversion_format fmt i conv
    | 't' ->
        let printer = get_arg spec n
        in cont_t (next_index spec n) printer (succ i)
    | ('l' | 'n' | 'L' as conv) ->
        (match Sformat.unsafe_get fmt (succ i) with
         | 'd' | 'i' | 'o' | 'u' | 'x' | 'X' ->
             let i = succ i in
             let s =
               (match conv with
                | 'l' ->
                    let (x : int32) = get_arg spec n
                    in format_int32 (extract_format fmt pos i widths) x
                | 'n' ->
                    let (x : nativeint) = get_arg spec n
                    in format_nativeint (extract_format fmt pos i widths) x
                | _ ->
                    let (x : int64) = get_arg spec n
                    in format_int64 (extract_format fmt pos i widths) x)
             in cont_s (next_index spec n) s (succ i)
         | _ ->
             let (x : int) = get_arg spec n in
             let s = format_int (extract_format_int 'n' fmt pos i widths) x
             in cont_s (next_index spec n) s (succ i))
    | ('{' | '(' as conv) -> (* ')' '}' *)
        let (xf : ('a, 'b, 'c, 'd, 'e, 'f) format6) = get_arg spec n in
        let i = succ i in
        let j = sub_format_for_printf conv fmt i
        in
          if conv = '{'
          then (* '}' *)
            (* Just print the format argument as a specification. *)
            cont_s (next_index spec n) (summarize_format_type xf) j
          else
            (* Use the format argument instead of the format specification. *)
            cont_m (next_index spec n) xf j
    | (* '(' *) ')' -> cont_s n "" (succ i)
    | conv -> bad_conversion_format fmt i conv
  in scan_positional n [] (succ pos)
...
let mkprintf to_s get_out outc outs flush k fmt =
  ...
  let rec pr k n fmt v =
    ...
    let rec doprn n i =
      if i >= len
      then Obj.magic (k out)
      else
        (match Sformat.unsafe_get fmt i with
         | '%' -> scan_format fmt v n i cont_s cont_a cont_t cont_f cont_m
         | c -> (outc out c; doprn n (succ i)))
    and cont_s n s i = (outs out s; doprn n i)
    and cont_a n printer arg i =
      (if to_s
       then outs out ((Obj.magic printer : unit -> _ -> string) () arg)
       else printer out arg;
       doprn n i)
    and cont_t n printer i =
      (if to_s
       then outs out ((Obj.magic printer : unit -> string) ())
       else printer out;
       doprn n i)
    and cont_f n i = (flush out; doprn n i)
    and cont_m n xf i =
      let m = Sformat.add_int_index (count_arguments_of_format xf) n
      in pr (Obj.magic (fun _ -> doprn m i)) n xf v
    in doprn n 0 in
  let kpr = pr k (Sformat.index_of_int 0) in kapr kpr fmt
...
let get_buff fmt = let len = 2 * (Sformat.length fmt) in Buffer.create len
  
let get_contents b = let s = Buffer.contents b in (Buffer.clear b; s)
  
let get_cont k b = k (get_contents b)
  
let ksprintf k =
  mkprintf true get_buff Buffer.add_char Buffer.add_string ignore
    (get_cont k)
